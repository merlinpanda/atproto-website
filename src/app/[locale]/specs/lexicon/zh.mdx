export const metadata = {
  title: 'Lexicon',
  description: 'ATProto的模式定义语言',
}

# Lexicon

Lexicon 是一种模式定义语言，用于描述ATProto记录、HTTP端点（XRPC）和事件流消息。它建立在ATProto[数据模型](/specs/data-model)之上。{{ className: 'lead' }}

该模式语言类似于[JSON Schema](http://json-schema.org/)和[OpenAPI](https://en.wikipedia.org/wiki/OpenAPI_Specification)，但包含一些ATProto特有的功能和语义。{{ className: 'lead' }}

本文档描述Lexicon定义语言的第1版规范。{{ className: 'lead' }}

## 类型概览

| Lexicon 类型    | 数据模型类型   | 类别       |
|----------------|-------------|------------|
| `null`         | Null        | 具体类型    |
